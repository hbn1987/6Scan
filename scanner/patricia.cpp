...
int Patricia::parsePrefix(std::string line, std::string *net) {
    std::string::size_type first, last;
    first = line.find_first_of("0123456789abcdef");
    last = line.find_last_of("0123456789abcdef");
    *net = line.substr(first,last-first+1);
    if (net->find_first_not_of("0123456789abcdef:/") != std::string::npos) {
...
void Patricia::populate(int family, std::ifstream& infile) {
    populate(family, infile, true);
}

void Patricia::populateBlock(int family, std::ifstream& infile) {
    populate(family, infile, false);
}

/*  we have two types of entries in the table:
 *  0 => prefix is alias prefixes
 *  ASN => prefix's AS number
 */
void Patricia::populate(int family, std::ifstream& infile, bool alias) {
    std::string line;
...
//    std::ifstream infile;
//    infile.open("./download/aliased_prefixes_202147");
//    tree->populate6(infile);
//    infile.close();
//    int *alias = NULL;
//    alias = (int *) tree->get(AF_INET6, "2600:1480:a000::1");
//    alias = (int *) tree->get(AF_INET6, "2600:1480:a000:410:1::100");
//    alias = (int *) tree->get(AF_INET6, "2607:f380:804:410:1::100");
//    if (alias)
