...
/*
 * Function: is_letter_iid()
 *
 * Determine if there is a letter in an IPv6 Interface ID
 */

unsigned int is_letter_iid(struct in6_addr *ipv6){
	unsigned int i, letter=0;
	uint8_t value = 0x09;
	for(i=8; i<=15; i++){
		if((ipv6->s6_addr[i] >> 4) > value || (ipv6->s6_addr[i] & 0x0f) > value)
			letter++;	
	}
	return(letter);
}
...
void decode_ipv6_address(struct decode6 *addr){
	...
	if(IN6_IS_ADDR_UNSPECIFIED(&(addr->ip6))){
		...
		else if( (ntohl(addr->ip6.s6_addr32[2]) >> 16) <= 0x255 && (ntohl(addr->ip6.s6_addr32[2]) >> 16) != 0 && \
				 (ntohl(addr->ip6.s6_addr32[2]) & 0x0000ffff) <= 0x255 && (ntohl(addr->ip6.s6_addr32[3]) >> 16) <= 0x255 && \
				 (ntohl(addr->ip6.s6_addr32[3]) & 0x0000ffff) <= 0x255 && is_letter_iid(&(addr->ip6)) == 0){
			addr->iidtype= IID_EMBEDDEDIPV4;
