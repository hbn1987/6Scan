...
size_t UdpProberIpv6::packProbe(const IpAddress& destinationIp,
  ...
  uint32_t packetExpectedSize =
      IP6_HDRLEN + ICMP_HDRLEN + trace6_HDRLEN + payloadMessage_.size();

  // Copy data right next to the trace6 header.
  memcpy(packet->payload + trace6_HDRLEN, payloadMessage_.c_str(),
         payloadMessage_.size());

  packet->trace6Header.initialTtl = ttl;
  packet->trace6Header.timestamp = timestamp;
  packet->trace6Header.probeStatus = probePhaseCode_;

  packet->trace6Header.prefixLen = 32;
...
// fix the UDP probe with varying prefixLen
size_t UdpProberIpv6::packProbe(const IpAddress& destinationIp,
                            const IpAddress& sourceIp, const uint8_t ttl,
                            uint8_t* packetBuffer, uint32_t prefixLen) {
  absl::uint128 destinationIpDecimal =
      (dynamic_cast<const Ipv6Address&>(destinationIp)).getIpv6Address();
  absl::uint128 sourceIpDecimal =
      (dynamic_cast<const Ipv6Address&>(sourceIp)).getIpv6Address();
  ...
  packet->trace6Header.prefixLen = prefixLen;
...
void UdpProberIpv6::parseResponse(uint8_t* buffer, size_t size,
  ...
  uint32_t sentTimestamp = residualUdpPacket->trace6Header.timestamp;
  uint8_t probePhase = residualUdpPacket->trace6Header.probeStatus;
  int16_t initialTTL = static_cast<int16_t>(residualUdpPacket->trace6Header.initialTtl);

  uint32_t prefixLen = residualUdpPacket->trace6Header.prefixLen;
  ...
  if (parsedPacket->icmp.icmp6_type == 1 &&
    ...
    fromDestination = false;
    distance = initialTTL;
  } else if (parsedPacket->icmp.icmp6_type == 129 &&
             parsedPacket->icmp.icmp6_code == 0) {
    // Echo Reply
    fromDestination = true;
    distance =
  ...
  Ipv6Address ipv6Destination(destination);
  Ipv6Address ipv6Responder(responder);

  (*callback_)(ipv6Destination, ipv6Responder, static_cast<uint8_t>(distance),
               rtt, fromDestination, false, buffer, size, prefixLen);
}
