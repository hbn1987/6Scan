...
#include "trace6/dcb.h"
#include "trace6/network.h"
#include "trace6/prober.h"
#include "trace6/udp_idempotent_prober.h"
#include "trace6/udp_prober.h"
#include "trace6/udp_prober_v6.h"
      ...
      if (networkManager_ != nullptr) {
        ...
        if (lastSeenSentPackets != 0 && lastSeenReceivedPackets != 0) {
          LOG(INFO) << boost::format(
                           "ProbingIterationRounds: %d SendingSpeed: %5.2fk ReceivingSpeed: %5.2fk \n"
                           "liveDcbSize: %5.2f BackwardProbingStopSetSize: %d ForwardProbingDiscoverySetSize: %d") %
...
void Tracerouter::startScan(ProberType proberType, bool ipv4,
  ...
  // LOG(INFO) << "There are " << dcbManager_->size() << " targets to probe.";
...
void Tracerouter::startPreprobing(ProberType proberType, bool ipv4) {
  ...
  PacketReceiverCallback callback =
      [this](const IpAddress& destination, const IpAddress& responder,
             ...
             void* receivedPacket, size_t packetLen, uint32_t prefixLen) {
...
void Tracerouter::startProbing(ProberType proberType, bool ipv4) {
  ...
  PacketReceiverCallback callback = [this](const IpAddress& destination,
                                           ...
                                           void* receivedPacket,
                                           size_t packetLen, uint32_t prefixLen) {
    if (parseIcmpProbing(destination, responder, distance, fromDestination, prefixLen) &&
        resultDumper_ != nullptr) {
      resultDumper_->scheduleDumpData(destination, responder, distance, rtt,
                                      fromDestination, ipv4, receivedPacket,
                                      packetLen);
  ...
  for (int scanCount = 0; scanCount < scanCount_ && !stopProbing_;
    ...
    do {
      ...
      // LOG(INFO) << parseIpFromIpAddressToString(*dcb.ipAddress) <<" "<< (int)nextBackwardTask
      //   <<" "<< (int)nextForwardTask <<" "<<dcb.prefixLen_;
      ...
      if (!hasBackwardTask &&
        ...
        if (forwardProbingMark_ && hasForwardTask) {
          ...
          networkManager_->scheduleProbeRemoteHost(*dcb.ipAddress,
                                                   nextForwardTask, dcb.prefixLen_);
        }
        if (hasBackwardTask) {
          // backward probing
          networkManager_->scheduleProbeRemoteHost(*dcb.ipAddress,
                                                   nextBackwardTask, dcb.prefixLen_);
        }
      }
...
bool Tracerouter::parseIcmpProbing(const IpAddress& destination,
                                   ...
                                   bool fromDestination, uint32_t prefixLen) {
  ...
  if (!fromDestination) {
    ...
    if (dcb->initialBackwardProbingTtl < distance) {
      // The response is from forward probing, the distance is error if forward probing is not activated.
      if (forwardProbingDiscoverySet_.find(&(const_cast<IpAddress&>(responder))) ==
          forwardProbingDiscoverySet_.end()) {
        forwardProbingDiscoverySet_.insert(responder.clone());
        dcbManager_->addDcbsFromReply(destination, distance, prefixLen);
      }
    } else {
      ...
      if (backwardProbingStopSet_.find(&(const_cast<IpAddress&>(responder))) !=
        ...
        dcbManager_->addDcbsFromReply(destination, defaultSplitTTL_, prefixLen);
      }
    }
