...
#include "trace6/bounded_buffer.h"
...
void NetworkManager::probeRemoteHost(const IpAddress& destinationIp,
                                     const uint8_t ttl, const uint32_t prefixLen) {
  static uint8_t buffer[kPacketBufferSize];
  size_t packetSize =
      prober_->packProbe(destinationIp, *localIpAddress_, ttl, buffer, prefixLen);
...
void NetworkManager::scheduleProbeRemoteHost(const IpAddress& destinationIp,
  ...
  if (expectedRate_ >= 1) {
    if (destinationIp.isIpv4()) {
      ...
      // handle IPv6.
      ProbeUnitIpv6 tmp(dynamic_cast<const Ipv6Address&>(destinationIp), ttl, prefixLen);
...
#else
    LOG(FATAL) << "trace6's IPv6 scan is not competible with the OS.";
#endif
...
void NetworkManager::runSendingThread() {
  ...
  while (!isStopReceiving()) {
    ...
    if (ipv4_) {
      ...
      probeRemoteHost(tmp6.ip, tmp6.ttl, tmp6.prefixLen);
    }
    sentProbes += 1;
  }
